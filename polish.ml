...
let string_to_expr x = try Num (int_of_string x) with int_of_string -> if x="" then failwith "Empty Var" else Var x;;
...
let rec line_to_instr l = match l with
